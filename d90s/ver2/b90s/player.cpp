...
u16 idletimer = 0;
u8 APlayer::Move(){
		u8 spriteanimframe = PA_GetSpriteAnimFrame(1,this->id);
	if(Pad.Held.Right) {
		this->vx = speedcap;
		this->hflipSprite(false);
		if(!(spriteanimframe>0&&spriteanimframe<=5))PA_StartSpriteAnim(1,this->id,1,5,10);
	}
	...
	if(checkCollisionRight(this->sx+21 + this->vx,this->sy,0) == this->co||
	checkCollisionRight(this->sx+8 + this->vx,this->sy,0)==this->co){
		for(int i = 0; i < this->vx; i ++){
			if(checkCollisionRight(this->sx +22 + distance,this->sy,0) == 0&&
			checkCollisionRight(this->sx +8 - distance,this->sy,0) == 0) distance ++;
		}
		this->x += distance;
		distance = 0;
		this->vx = 0;
	...
	if(Pad.Held.Y || Pad.Held.X){
		PA_SetSpriteAnimSpeed(1,this->id, 14);
		this->speedcap = 4;
	}
	if(!(Pad.Held.Y || Pad.Held.X)) {
		PA_SetSpriteAnimSpeed(1,this->id, 10);
		this->speedcap = 2;
	}
	if(!(Pad.Held.Right || Pad.Held.Left)) {
		this->vx = 0;
...
u8 APlayer::Gravity(){
	...
	(pixel == this->bp && this->g >= 0)){
		...
		&& !(Pad.Held.Right || Pad.Held.Left)) {
			PA_SetRotsetNoAngle(1,0,194,192);
			PA_SetSpriteAnimFrame(1,this->id,7);
			idletimer = 0;
		}
...
void APlayer::Update(){
    ...
    PA_SetSpriteXY(1,this->id,this->sx - 17,this->sy - 22);
