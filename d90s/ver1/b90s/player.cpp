...
u16 idletimer = 0;
u8 APlayer::Move(){
	if(checkCollisionRight(this->sx+22 + this->vx,this->sy,0) == this->co||
	checkCollisionRight(this->sx+8 + this->vx,this->sy,0)==this->co){
		for(int i = 0; i < this->vx; i ++){
			if(checkCollisionRight(this->sx +22 + distance,this->sy,0) == 0) distance ++;
		}
		this->x += distance;
		distance = 0;
		this->vx = 0;
	...
	if(Pad.Newpress.Left||Pad.Newpress.Right)PA_StartSpriteAnim(1,this->id,1,4,15);
	
	if(Pad.Held.Y || Pad.Held.X)this->speedcap = 4;
	
	if(!(Pad.Held.Y || Pad.Held.X)) this->speedcap = 2;
